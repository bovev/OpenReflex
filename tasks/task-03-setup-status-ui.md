...
## Do
- Show service readiness, offline-ready state, active/default profile, installed revision, verification time, download size, disk usage, and model state from `/v1/status` and `/v1/models`.
...
- Start downloads only after an explicit button action. Poll status while downloading and display byte progress, current phase, completion, failure, and retry/verify actions.
...
- Test not-installed, downloading, installed/offline-ready, damaged, interrupted, API-error, retry, and refresh-recovery states with deterministic fixtures.
